...
module.exports = (db) => ({
    ...
    async updateUserTokenById(userId, token) {
        const result = await db.queryParams(UPDATE_USER_TOKEN_BY_ID, [userId, token])
        return result.rows[0]
    },

    async saveUser(firstName, lastName, avatar, login, password, accessToken) {
        const result = await db.queryParams(INSERT_USER, [firstName, lastName, avatar, login, password, accessToken])
        return result.rows[0]
    }
})